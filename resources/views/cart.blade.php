                  ...
                  <tbody>
                    ...
                    <tr>
                      <form action="{{ route('cart.remove') }}" method="POST">
                        @csrf
                        <input type="none" name="id" value="{{ $item->id}}" style="display: none;">
                        <!-- <td><a class="remove" href="javascript:;" onclick="parentNode.submit();"><fa class="fa fa-close"></fa></a></td> -->
                        <td style="display: none;"><button type="submit" class="px-4 py-2 text-white bg-red-600">x</button></td>
                      </form>
                        <td><a href="#"><img src="/uploads/products/{{ $item->attributes->image }}" alt="img"></a></td>
                        <td><a class="aa-cart-title" href="#">{{ $item->name }}</a></td>
                        <td>RM{{ $item->price }}</td>
                      <form action="{{ route('cart.update') }}" method="POST">
                        @csrf
                        <input type="hidden" name="id" value="{{ $item->id}}" >
                        <td><input class="aa-cart-quantity" name="quantity" type="number" min="0" value="{{ $item->quantity }}" onchange='this.form.submit()'></td>
